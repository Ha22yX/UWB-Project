...
void mavlinkSendSetpointGlobalRelAlt(int32_t lat_e7, int32_t lon_e7, float rel_alt_m) {
  mavlink_message_t msg;
  // ignore velocity/accel/yaw, use position only
  const uint16_t type_mask =
      (uint16_t)(
          (1 << 3) | (1 << 4) | (1 << 5) | // vx, vy, vz
          (1 << 6) | (1 << 7) | (1 << 8) | // ax, ay, az
          (1 << 10) | (1 << 11));          // yaw, yaw_rate
...
void mavlinkSendSetpointGlobalRelAltYaw(int32_t lat_e7, int32_t lon_e7, float rel_alt_m, float yaw_deg) {
  ...
  const uint16_t type_mask =
      (uint16_t)(
          ...
          (1 << 6) | (1 << 7) | (1 << 8) | // ax, ay, az
          (1 << 11));                      // yaw_rate
  const float yaw_rad = yaw_deg * 0.01745329252f;
  mavlink_msg_set_position_target_global_int_pack(
      SYS_ID, COMP_ID, &msg,
      millis(),
      TARGET_SYS, TARGET_COMP,
