...
// Median filter (per-anchor)
static const int FILTER_N = 1;
...
bool solve3DWithMirror(float &x, float &y, float &z) {
  ...
  z = zsum / zcount; // positive mirror (above plane)
  return true;
}
...
void setup() {
  ...
  Serial.println("ESP32 #2 UWB tag solver (4 anchors)");
  printHelp();

  sendAT("AT+anchor_tag=0");
  delay(100);
  sendAT("AT+interval=2");
  delay(100);
